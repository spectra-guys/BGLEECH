    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                "Hey Your File Size is Bigger Than Usual\n"
                f"Found its of: {d_f_s} 😡\n"
                "\nDon't do it again your file is splited into parts."
            )
            ...
            await i_m_s_g.edit_text(
                f"Found File Size is : {d_f_s} 😡\n"
                f"<code>{ba_se_file_name}</code> Destroyed into {number_of_files} files.\n"
                "Uploading to Telegram and Make sure don't do this again..🥺"
            )
    ...
    try:
        ...
        if not edit_media:
            message_for_progress_display = await message.reply_text(
                "Uploading Started!!!...😃 {}".format(os.path.basename(local_file_name))
            )
        ...
        await message_for_progress_display.edit_text("INVALID ERROR!!\n" + str(e))
    else:
